...
export default function CollectionContent({ type }: CollectionContentProps) {
  ...
  const fetchCollection = useCallback(async (pageNum: number = 1, sort: 'default' | 'price_asc' | 'price_desc' = 'default') => {
    ...
    try {
      ...
      const response = await fetch(`/api/collections/${type}?${params.toString()}`, { 
        cache: 'no-store',
  ...
  }, [type, sortOrder, fetchCollection])
      ...
      <main className="flex-1">
        ...
        {collection && (
          <section 
            ...
            style={{ 
              backgroundColor: theme.background || (type === 'best' ? '#F3E9D7' : type === 'sale' ? '#FFE5E5' : '#F5F0E8')
            }}
          >
            <div className="container mx-auto px-4 text-center">
              <h1 
                className="text-4xl font-bold mb-2"
                style={{ color: theme.title_color || '#2A2A2A' }}
              >
                {collection.title || (type === 'best' ? '베스트' : type === 'sale' ? '특가' : '한우대가 NO.9')}
              </h1>
              {collection.description && (
                <p 
                  className="text-sm tracking-widest opacity-90 mt-2"
